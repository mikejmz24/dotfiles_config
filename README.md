...
### Theme: Nordic (dark) with custom navy accent `#1a3a6b`

Nordic is a dark GTK theme with a grey-blue palette. We override its default
teal accent (`#8fbcbb`) with a deep navy (`#1a3a6b`) across all GTK layers
and Firefox.

### Firefox manual step (one-time, already done)

After running `install-linux.sh`, open Firefox and go to `about:config`:
toolkit.legacyUserProfileCustomizations.stylesheets = true

Restart Firefox. Text selection will now match the system navy accent.

### Why so many files needed patching

GNOME's GTK theming stack has 4 independent layers — each must be patched
separately:

| Layer           | File                                            | What it controls               |
| --------------- | ----------------------------------------------- | ------------------------------ |
| GTK2            | `gtk-2.0/gtkrc`                                 | Legacy apps                    |
| GTK3            | `gtk-3.0/gtk.css` + `~/.config/gtk-3.0/gtk.css` | Most native apps               |
| GTK4/libadwaita | `gtk-4.0/gtk.css` + `~/.config/gtk-4.0/gtk.css` | Modern GNOME apps              |
...
| `Graphite-blue-Dark`      | Accent hardcoded as Google blue (`#1A73E8`) in multiple layers |
